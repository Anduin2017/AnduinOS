...
var MyShowAppsIconMenu = class extends PopupMenu.PopupMenu {
    ...
    updateItems(sourceActor) {
        ...
        if(this.sourceActor == Main.layoutManager.dummyCursor) {
            this._appendSeparator();
            let item = this._appendMenuItem(this._dtpPanel._restoreWindowList ? _('Restore Windows') : _('Show the Desktop'));
        ...
        // Lock the taskbar
        let lockTaskbarMenuItem = this._appendMenuItem(Me.settings.get_boolean('taskbar-locked') ? _('Unlock the taskbar') : _('Lock the taskbar'));
    ...
    _appendList(commandList, titleList) {
        console.log(commandList, titleList);
        ...
        for (var entry = 0; entry < commandList.length; entry++) {
            console.log("appending item with title: " + titleList[entry] + " and command: " + commandList[entry]);
            this._appendItem({
                title: titleList[entry],
                cmd: commandList[entry]
            });
        }
    }
